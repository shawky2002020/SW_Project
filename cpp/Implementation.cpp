...
void UIManager::displayAdminMenu()
{
    ...
    cout << "2. Close Customer Account\n";
    cout << "3. Search for Customer Account\n";
    cout << "4. Logout\n";
    cout << "------------------------\n";
}
...
bool Customer::login(string user, string pass)
{
    if (isLocked)
    {
        UIManager::showError("Account is locked due to too many failed attempts.");
        return false;
    }
    while (failedLoginAttempts < 5)
    {
        ...
        {
            failedLoginAttempts++;
            UIManager::showError("Failed attempts: " + to_string(failedLoginAttempts) + "/5");

            if (failedLoginAttempts >= 5)
            {
                isLocked = true;
                UIManager::showError("Account locked. Too many failed attempts.");
            }
            else
            {

                user = UIManager::getInput("Enter username: ");
                pass = UIManager::getInput("Enter password: ");
            }
        }
    }
    UIManager::showError("Account is locked due to too many failed attempts.");
...
void BankSystem::run()
{
    ...
    if (authenticateUser(user, pass))
    {
        ...
        {
            adminLoop(static_cast<Admin *>(currentUser));
        }
    }
...
void BankSystem::adminLoop(Admin* admin)
{
...
void BankSystem::customerLoop(Customer *customer)
{
    ...
    while (running)
    {
        ...
        case 4:
        { // Transfer (Simplified: Transfer to a hardcoded external account)
